...
export default function ImageUploader({reqdetails}) {
...
if (extFile == "jpg" || extFile == "jpeg" || extFile == "png" || extFile == "svg" || extFile == "gif") {
   //DO Code here after image format verification
   const dataArray = new FormData();
    ...
    axios.post(`${base_url}/upload`,dataArray,{
      headers: {
        "Content-Type": "multipart/form-data"            //change header to receive image 
      }
    }).then(
      (response) => {
  ...
  return (
    <div className="App">
      <form onSubmit={submitForm}>
        ...
        <input type="file" onChange={(e) => setUploadFile(e.target.files)} multiple="false" accept="image/png, image/jpeg "/>
        <br />
        {imageuploadstatus?(`file uploaded already` ):(<input type="submit" disabled={disablebutton} />)}
